...
for i=(1+yHalfBox):STEP:(rows-yHalfBox)
    ...
    for j=(1+xHalfBox):STEP:(cols-xHalfBox)
        if (mask(i,j))
            ...
            whiteInput = colourRatioWhite(box,white,whiteThreshold);
            inputs = [ inputs [redInput; jeansInput; whiteInput] ];
            index = [ index [i;j] ];
        end
    end
    if sum(mask(i,:))>0
...
for ii=1:length(inputs)
    ...
    whiteRatio = inputs(3,ii);
    i = index(1,ii);
    j = index(2,ii);
    
        if (out>THRESHOLD)
            fprintf('WALLY FOUND!(%i,%i) (%d%%)\t',i,j,round(out*100));
            fprintf('reds:%d%%\t jeans:%d%% white:%d%%\n',round(redRatio*100),round(jeansRatio*100),round(whiteRatio*100));
            down = (i-yHalfBox);
            top = (i+yHalfBox);
            left = (j-xHalfBox);
            right = (j+xHalfBox);
