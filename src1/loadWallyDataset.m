...
for i=1:N_POSITIVE_PATTERNS
    ...
    inputs = [inputs [ratioRed; ratioJeans; ratioSkin] ];
    outputs = [outputs 1];
end
fprintf('%d positive patterns loaded\n',N_POSITIVE_PATTERNS);

% loading negative patterns
patternsList = dir('../negativePatterns/*.JPG');
[N_NEGATIVE_PATTERNS row] = size(patternsList);


for i=1:N_NEGATIVE_PATTERNS
    name = strcat('../negativePatterns/',patternsList(i).name);
    %fprintf('loading pattern: %s\n',name);
    pattern = imread(name);
